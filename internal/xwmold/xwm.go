...
type Command interface {
	Handle(ctx context.Context, conn *xgb.Conn, state *State) error
}

var ErrQuit = fmt.Errorf("quit")

func Run(ctx context.Context, conn *xgb.Conn, state State, eventC <-chan any) error {
	for {
		var msg any
		...
		command := Next(ctx, conn, state)
		err := command.Handle(ctx, conn, &state)
		if err != nil {
